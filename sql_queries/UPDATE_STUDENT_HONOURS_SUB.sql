DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `UPDATE_STUDENT_HONOURS_SUB`(
...
BEGIN
	START TRANSACTION;
			...
			INSERT INTO academic_record (ACADEMIC_YEAR,STUDENT_SEMESTER_ID,COURSE,PAPER,SUB_CODE) 
				SELECT sem.SEM_YEAR, @LAST_UPDATED_ID,"CORE", p.PAPER, p.SUB_CODE FROM student_semester sem, papers p 
					WHERE p.COURSE = "CORE" AND p.SUB = honours_sub AND p.SEMESTER = 1 AND sem.ID = @LAST_UPDATED_ID;
	COMMIT;
END$$
DELIMITER ;
